  ...
  Status Compute(OpKernelContext* context) const override {
    ...
    if (scales_.empty()) {
        ...
        if (scales != nullptr && scales->Shape().Size() != 0) {
            ...
            memcpy(Y_shape.data(), sizes->template Data<int64_t>(), sizes->Shape().Size() * sizeof(int64_t));
    ...
    if (mode_ == UpsampleMode::LINEAR) {
      ...
      ZoneNamedN(_tracy_ResizeBilinear2D, "ResizeNearest2D (kernel launch)", true);
      ORT_RETURN_IF_ERROR(
          KernelLauncher{GetKernel("ResizeNearest2D")}
              .setInt2(desc.Width(), desc.Height())
              .setImage2Ds(*X, *Y)
              .setInt2(X_shape[3], X_shape[2])
              .setInt2(Y_shape[3], Y_shape[2])
