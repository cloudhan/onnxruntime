...
get cmatrix stride
*/
FBshape GetStridesY(DirectBuffer_ptr matrix) {
...
WinogradHelper::WinogradHelper(int computeUnit, int kernelSize) {
  ...
  G_ = DirectBuffer_ptr(new DirectBuffer);
...
*/
DirectBuffer_ptr WinogradHelper::allocWeightTensor(int ochannel, int ichannel, int unitCi, int unitCo) {
  int ciC4 = opencl::CeilDiv(ichannel, unitCi);
  int coC4 = opencl::CeilDiv(ochannel, unitCo);
  DirectBuffer_ptr p = DirectBuffer_ptr(new DirectBuffer);
  p->create({16, coC4, ciC4, unitCi, unitCo});
  return p;
}

// Winograd 4x4 For Conv 3x3
static inline void WeightTransform4x4_3x3(const float* src, float* dst, DirectBuffer_ptr G, const FBshape& weight_dest_strides,
...
*/
DirectBuffer_ptr WinogradHelper::transformWeight(const float* source, int output_channel, int input_channel) {
  DirectBuffer_ptr weightDest_ptr = allocWeightTensor(output_channel, input_channel, 4, 4);
